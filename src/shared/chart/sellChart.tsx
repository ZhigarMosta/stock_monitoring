...
import { $sellChart } from "../../store/chartsStore";

am4core.useTheme(am4themes_animated);

const SellChart = () => {
  const dataChart = useUnit($sellChart);
  const data = dataChart[0].bids;

  const chartHeight = data.length * 41;
  ...
  useLayoutEffect(() => {
    ...
    xAxis.min = 0; // Минимальное значение
    xAxis.max = 100; // Максимальное значение

    let series = chart.series.push(new am4charts.ColumnSeries());
    series.dataFields.valueX = "value";
    series.dataFields.categoryY = "prise";
    series.name = "Значение";
    series.columns.template.fill = am4core.color("#6B77BE");
    ...
    //   const originalValue = target.dataItem.dataContext.value;
    //   const invertedValue =
